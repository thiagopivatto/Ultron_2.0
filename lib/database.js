...
const conversationFilePath = path.resolve('./database/json/chatgpt.json');
...
// Obter o histórico da conversa do Redis
    ...
    return new Promise((resolve, reject) => {
      redisClient.lrange(chatId, 0, -1, (error, conversation) => {
        if (error) {
          ...
          const parsedConversation = conversation.map((msg) => JSON.parse(msg));
