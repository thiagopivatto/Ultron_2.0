...
module.exports = {
    ...
    criarArquivosNecessarios : async ()=>{
        try {
          ...
          const existeBotJson = fs.existsSync(path.resolve("database/json/bot.json")), existeEnv = fs.existsSync(path.resolve('.env')), existeAntiflood = fs.existsSync(path.resolve('database/json/antiflood.json')), existeChatGPT = fs.existsSync(path.resolve('database/json/chatgpt.json'))
          if(existeBotJson && existeEnv && existeAntiflood && existeChatGPT) return false
          if(!existeBotJson){
          ...
          if(!existeChatGPT){
            await fs.writeFile(path.resolve("database/json/chatGPT.json"), "[]")
          }
          return true
        } catch(err){
            throw new Error(err)
