...
|- *!transcreva* - Responda um audio para transformar em texto (EM DESENVOLVIMENTO).
|- *!dalle* - Cria uma imagem com o texto que você digitar (EM DESENVOLVIMENTO).
|- *!simi* - Alterna o estado do Sim Simi para conversar com ele (EM DESENVOLVIMENTO).
|- *!gerar* - Gera imagens através de uma IA (EM DESENVOLVIMENTO).
|
╰╼❥ ${process.env.NOME_BOT.trim()}® by *${process.env.NOME_ADMINISTRADOR.trim()}*`
    },
...
|- *!vddousaf* - [vdd/dsf] [nivel(1-5)] [random(opcional)] - Exibe uma pergunta aleatória para responder com *vdd* ou *dsf*."
|
╰╼❥ ${process.env.NOME_BOT.trim()}® by *${process.env.NOME_ADMINISTRADOR.trim()}*`
        } else {
            return `__| ☾ *🤖 ${process.env.NOME_BOT.trim()}®* ☽ 
