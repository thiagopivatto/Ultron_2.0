...
// Método para chamada do ChatGPT
const callChatGPT = async (chatId, userMessage) => {
    ...
    const params = {
        messages: conversation.concat([{ role: 'system', content: userMessage }]),
      };
  
    const response = await openaiInstance.complete(params);
    const responseData = response.choices[0].message.content;
    await saveMessage(chatId, 'user', userMessage);
    await saveMessage(chatId, 'assistant', responseData);
    return responseData;
  };
  
// Obter o histórico da conversa a partir do Redis
const getConversation = async (chatId) => {
    return new Promise((resolve, reject) => {
      redisClient.lrange(chatId, 0, -1, (error, conversation) => {
        if (error) {
          console.error('Erro ao obter a conversa do Redis:', error);
          resolve([]);
        } else {
...
const saveMessage = async (chatId, sender, message) => {
    ...
    redisClient.rpush(chatId, JSON.stringify(newMessage), (error) => {
      if (error) {
        console.error('Erro ao salvar a mensagem no Redis:', error);
      }
    });
  };
  ...
  module.exports = {
    ...
    callChatGPT: callChatGPT, redisClient,
