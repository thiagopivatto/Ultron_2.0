...
headers.append('Authorization', `Bearer ${apiKey}`)
  ...
  module.exports = {
    ...
    //========= INÍCIO CONFIGURAÇÃO CHAT GPT 3==== api.js ======//
    callChatGPT: async (userMessage, message) =>{
        const chatID = message.from;
        const params = {
            content: userMessage,
        };
        const response = await axios.post(`https://us-central1-chatgpt-api-pivatto.cloudfunctions.net/ChatGPT-API/${chatID}/chat`, params);         
        const data = await response.data;
        return data.content;
    },
    //========= FINAL  CONFIGURAÇÃO CHAT GPT 3==== api.js ======// 

    callSimSimi: async (userMessage) => {
        const apiKey = process.env.SIMSIMI_API_KEY;
        const apiUrl = 'http://sandbox.api.simsimi.com/request.p';
        const headers = {
        ...
        if (response.status === 200 && data.atext) {
          return data.atext;
        } else {
          console.error(data);
          throw new Error('Sim Simi não retornou uma resposta válida.');
        }
      },
      
      

    generateDALLEImage: async(textoImagem, qtdFotos) =>{
        const params = {
          ...
          const response = await fetch('https://api.openai.com/v1/engines/davinci/images', options);
          const data = await response.json();
          return data.images;
    },    
    
    traduzirFrases: async (frases) => {
        const { Translate } = require('@google-cloud/translate').v2;
        const keyFilename = path.resolve(__dirname, '..', 'database', 'json', 'chavegcloud.json');
        const translate = new Translate({keyFilename: keyFilename,});
        const targetLanguage = 'pt';
        for (let i = 0; i < frases.length; i++) {
          const frase = frases[i];
          const [translation] = await translate.translate(frase.summary, targetLanguage);
          frases[i].summary = translation;
        }
      },
    
      textoParaVoz: async (texto, voiceOptions) => {
        const { TextToSpeechClient } = require('@google-cloud/text-to-speech');
        const ttsClient = new TextToSpeechClient({
            credentials: {
            private_key: process.env.GOOGLE_PRIVATE_KEY_TTS.replace(/\\n/g, '\n'),
            client_email: process.env.GOOGLE_CLIENT_EMAIL_TTS,
            },
        });
        const request = {
          input: { text: texto },
        ...
        try {
          ...
          const outputDir = path.join(__dirname, '..', 'media', 'tts');
          if (!fs.existsSync(outputDir)) {
            fs.mkdirSync(outputDir, { recursive: true });
          }
      
          let audioFileName = 'voz.mp3';
          let audioPath = path.join(outputDir, audioFileName);
          let counter = 1;
          while (fs.existsSync(audioPath)) {
            audioFileName = `voz_${counter}.mp3`;
          ...
          if (err.details === 'MediaFileEmpty') {
            throw new Error('O arquivo de áudio está vazio.');
          } else if (err.details.includes('Requested female voice')) {
            // Solicitou voz feminina, mas não há suporte, então tentaremos voz masculina
            voiceOptions.ssmlGender = 'MALE';
            const audioPath = await module.exports.textoParaVoz(texto, voiceOptions);
            fs.unlinkSync(audioPath);
            return audioPath;
          } else {
            console.error(err);
            throw new Error('Ocorreu um erro ao gerar o áudio.');
          }
        }
      },   
      

    textoParaVoz2: async (idioma, texto)=>{
    ...
    // VERIFICAR AQUI
    obterAudioVocal: async (userMessage, message) =>{
        const chatID = message.from;
        const params = {
            content: userMessage,
        };
    ...
    obterAudioModificado: (caminhoAudio, tipoEdicao, vocal, instrumental) => {
        return new Promise((resolve, reject) => {
          ...
          switch (tipoEdicao) {
            ...
            case "vocal":
              comando = `python3 -m demucs.separate ${caminhoAudio} --out ${vocal} --dl`;
              break;
      
            case "instrumental":
              comando = `python3 -m demucs.separate ${caminhoAudio} --out ${instrumental} --dl`;
              break;
      
            default:
              reject(new Error('Tipo de edição inválido.'));
          }
          ...
          if (comando) {
            ...
            processo.on('error', (err) => {
              reject(new Error('Erro ao modificar o áudio.'));
            });
      
            processo.on('exit', (code) => {
              if (code === 0) {
                resolve({ vocal, instrumental });
              } else {
                reject(new Error('Erro ao modificar o áudio.'));
              }
            });
    ...
    obterClima: async local => {
        try {
            ...
            const climaFotoURL = `https://pt.wttr.in/${encodeURI(local)}.png`;
            const climaTextoURL = `https://pt.wttr.in/${encodeURI(local)}?format=Local%20=%20%l%0AClima%20atual%20=%20%C%0ATemperatura%20=%20%t%C2%B0C%0AUmidade%20=%20%h%0AVento%20=%20%w%0ALua%20agora%20=%20%m%0ANascer%20do%20Sol%20=%20%S%0APor%20do%20Sol%20=%20%s`;
    ...
    obterJogosDaData: async(data) => {
        const options = {
            method: 'GET',
            url: `https://sportscore1.p.rapidapi.com/teams/${data}`,
            params: {page: '1'},
            headers: {
    ...
    obterDadosDoTime: async(idTime) => {
        const options = {
            method: 'GET',
            url: `https://sportscore1.p.rapidapi.com/teams/${idTime}`,
            headers: {
            'X-RapidAPI-Key': process.env.X_RAPIDAPI_KEY,
            'X-RapidAPI-Host': process.env.X_RAPIDAPI_HOST
            }
    ...
    obterJogosDoTime: async(idTime) => {
        const options = {
            method: 'GET',
            url: `https://sportscore1.p.rapidapi.com/teams/${idTime}/events`,
            headers: {
            'X-RapidAPI-Key': process.env.X_RAPIDAPI_KEY,
            'X-RapidAPI-Host': process.env.X_RAPIDAPI_HOST
            }
    ...
    obterJogadoresDoTime: async(idTime) => {
        const options = {
            method: 'GET',
            url: `https://sportscore1.p.rapidapi.com/teams/${idTime}/players`,
            headers: {
            'X-RapidAPI-Key': process.env.X_RAPIDAPI_KEY,
            'X-RapidAPI-Host': process.env.X_RAPIDAPI_HOST
            }
    ...
    traduzirPosicao: async(posicao) =>{
        if (!posicao) {
            return ''; // Retorna uma string vazia se a posição for nula ou indefinida
          }
        switch (posicao) {
            case 'GK':
            return 'Goleiro';
            ...
            case 'CAM':
            return 'Meia Atacante Central';
            case 'LAM':
            return 'Meia Atacante Esquerdo';
            case 'RAM':
            return 'Meia Atacante Direito';
            case 'WL':
            ...
            return 'Centroavante';
            case 'LF':
            return 'Centroavante Esquerdo';
            case 'RF':
            return 'Centroavante Direito';
            case 'RS':
            return 'Centroavante';
            case 'LS':
            return 'Centroavante';
