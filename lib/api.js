...
/*async function callChatGPT(userMessage, chatHistory) {
  ...
  const context = getContext(chatHistory);
...
function getContext(chatHistory) {
  let context = '';
  for (const chat of chatHistory) {
    if (chat.role === 'system' || chat.role === 'user') {
      context += chat.content + '\n';
    }
  }
  return context.trim();
}*/
//========= FIM CONFIGURAÇÃO CHAT GPT 3==== api.js ======// 
